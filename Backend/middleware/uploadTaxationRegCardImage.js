...
const uploadTaxationRegCardImage = async (req, res, next) => {
    upload.single('taxationRegCard')(req, res, async (err) => {
      if (err) {
        return res.status(500).json({ error: "tax mw1" + err.message });
      }
      ...
      try {
        ...
        return res.status(500).json({ error: "tax mw" + error.message });
      }
    });
  };

module.exports = uploadTaxationRegCardImage;
