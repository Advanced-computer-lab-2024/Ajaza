...
exports.updateGovernor = async (req, res) => {
  try {
    const updatedGovernor = await Governor.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
...
exports.updateGovernorVenue = async (req, res) => {
  try {
    ...
    if (!venue) {
      return res
        .status(404)
        .json({ message: "Venue not found or not authorized to update" });
    }
...
exports.deleteGovernorVenue = async (req, res) => {
  try {
    ...
    if (!venue) {
      return res
        .status(404)
        .json({ message: "Venue not found or not authorized to hide" });
    }
  ...
  try {
    ...
    if (!validTags.includes(tag)) {
      return res.status(400).json({
        message:
          "Invalid tag. Valid tags are: Monuments, Museums, Religious Sites, Palaces/Castles.",
      });
    }
    ...
    if (!venue.tags.includes(tag)) {
      ...
      return res
        .status(400)
        .json({ message: "Tag already exists for this venue." });
    }
    ...
    res
      .status(200)
      .json({ message: "Tag assigned to venue successfully", tag, venue });
  } catch (error) {
