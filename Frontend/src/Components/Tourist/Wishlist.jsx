...
const Wishlist = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
        try {
          const wishlistResponse = await axios.get(`${apiUrl}tourist/wishlist/${touristId}`);
    ...
    const cardOnclick = (product) => {
      ...
      const constProps = { rateDisplay: true, currency, currencyRates: { EGP: 48.58, USD: 1, EUR: 0.91 } };
    ...
    useEffect(() => {
      ...
      const addToCartFromWishlist = async (element, quantity) => {
        console.log("Element passed to addToCartFromWishlist:", element); // Debugging

        const productId = element["_id"]; // Extract the product ID from the element
        try {
            if (!userid || !productId) {
            ...
            const response = await axios.post(`${apiUrl}tourist/add-to-cart-from-wishlist`, {
                touristId: userid,
                productId,
      ...
      const WishlistCard = ({ element, propMapping, onCartClick, ...props }) => {
        ...
        return (
            <div style={{ position: "relative" }}>
                ...
                <div
                    ...
                    onClick={() => onCartClick(element, quantity)} // Pass quantity along with element
                >
                    <ShoppingCartOutlined
                        style={{
                            marginLeft: "10px",
          ...
          {loading ? (
            ...
            cardComponent={(props) => (
              <div style={{ position: "relative" }}>
                <WishlistCard
                  {...props}
                  element={props.element} // Make sure element is passed
                  propMapping={propMapping}
                  onCartClick={(element, quantity) => addToCartFromWishlist(element, quantity)} // Pass quantity
                />
