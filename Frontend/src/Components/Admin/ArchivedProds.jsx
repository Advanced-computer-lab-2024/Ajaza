...
import { Modal, message, Card, Row, Col, Empty } from "antd";
...
const ArchivedProducts = () => {
  ...
  useEffect(() => {
    const fetchArchivedProducts = async () => {
      ...
      try {
        ...
        const archivedProductsList = products.filter(
  ...
  const unarchiveProduct = async (product) => {
    try {
      await axios.patch(
        `${apiUrl}product/${userId}/product/${product._id}/adminSellerArchiveProduct`,
        {
  ...
  return (
    <>
      <h2 style={{ marginBottom: 40 }}>Archived Products</h2>
          ...
          {archivedProducts.map((product) => (
            <Col span={6} key={product["_id"]}>
              <Card
                ...
                extra={
                  <UnarchiveIcon
                    onClick={() => showUnarchiveModal(product)}
      ...
      <Modal
        ...
        onOk={() => {
          const productToUnarchive = archivedProducts.find(
            (item) => item._id === unarchivingProductId
          );
