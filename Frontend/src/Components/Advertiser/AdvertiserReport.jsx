...
const AdvertiserReport = () => {
  ...
  useEffect(() => {
    ...
    if (token) {
      try {
        ...
        const AdvertiserIdFromToken =
          decoded?.userId || decoded?.AdvertiserId || null;
    ...
    const fetchSalesData = async () => {
      ...
      try {
        const response = await axios.get(
          `http://localhost:5000/advertiser/viewSalesReport/${AdvertiserId}`
        );
  ...
  const applyFilters = (currentData, currentFilters) => {
    ...
    if (
      currentFilters.activityNames &&
      currentFilters.activityNames.length > 0
    ) {
      filteredData = filteredData.filter((item) =>
        currentFilters.activityNames.some(
          (name) => item.activityName.toLowerCase().includes(name.toLowerCase()) // Check if activity name includes any selected name
        )
      );
    }
  ...
  const handleDateRangeChange = (dates, filterMode = "date") => {
    ...
    const newTotalSales = filteredData.reduce(
      (sum, item) => sum + item.price,
      0
    ); // Recalculate total sales from filtered data
  ...
  const uniqueActivityNames = [
    ...new Set(originalSalesData.map((item) => item.activityName)),
  ]; // Extract unique activity names for filters
  const columns = [
    {
      title: "Activity Name",
  ...
  return (
    <div>
      ...
      <p>
        Total Sales: <strong>${totalSales.toFixed(2)}</strong>
      </p>
      <div
        style={{
          display: "flex",
