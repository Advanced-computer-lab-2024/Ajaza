...
const Venues = () => {
  ...
  const editVenue = async (id) => {
    ...
    try {
      ...
      setVenuesData(venuesData.map(venue => 
        venue._id === id ? response.data : venue
      ));
  ...
  return (
    <div style={{ display: 'flex' }}>
      ...
      <div style={{ padding: '20px', flexGrow: 1 }}>
            ...
            {venuesData.map((venue) => (
              ...
              >
                <Card.Meta
                  ...
                  description={
                    <p>
                      {`Location: ${venue.location}, Price (Foreigner): ${venue.price.foreigner}, Price (Native): ${venue.price.native}, Tags: ${venue.tags.join(', ')}`}
                    </p>
                  }
                />
              </Card>
        ...
        <Modal title="Create Venue" visible={isModalVisible} onCancel={handleCancel} footer={null}>
          <Form form={form} layout="vertical" onFinish={createVenue}>
            <Form.Item name="name" label="Venue Name" rules={[{ required: true, message: 'Please input the venue name!' }]}>
              <Input />
            </Form.Item>
            <Form.Item name="desc" label="Description" rules={[{ required: true, message: 'Please input the venue description!' }]}>
              <Input />
            </Form.Item>
            ...
            <Form.Item name="price" label="Price" rules={[{ required: true, message: 'Please input the price!' }]}>
              <Input.Group compact>
                <Form.Item name={['price', 'foreigner']} noStyle>
                  <Input style={{ width: '50%' }} placeholder="Foreigner price" />
                </Form.Item>
                <Form.Item name={['price', 'native']} noStyle>
                  <Input style={{ width: '50%' }} placeholder="Native price" />
                </Form.Item>
              </Input.Group>
            </Form.Item>
